...
\textbf{等控制资源条件下的控制律对比：精度相当，差异在证书。}图~\ref{fig:hw-laws-tether30}（右，$k{=}36$、拴定 $30$）显示：同一拴定档内四律的终态与 p95 差异（位置 $\le2.4\,\mathrm{mm}$、姿态 $\le0.35^\circ$）已与单次运行的测量波动同量级。记录中的精度接近，说明 C1 的收益不在跟踪精度，而在\textbf{可核验的证书}：其余三律均无定理 3 意义下的证书（C2/C4 的位姿反馈不经 $A^\top$ 整形，C3 为一阶桥接）；C1 的记录信号满足增益和水平集条件，治理器介入帧按 $r_{\rm gov}$ 计为附加扰动，不声称连续时间证书仍严格成立。
...
\caption{等控制资源条件下的四律对比（$k{=}36$，单次运行，$10\,\mathrm{s}$ 标准轨迹）：左为 $K_{\mathrm{tr}}{=}2\,\mathrm{N\,m/rad}$（拴定近乎退出的纯度极限，准静态残差由摩擦死区决定、约 $32$--$40\,\mathrm{mm}$）；右为 $K_{\mathrm{tr}}{=}30\,\mathrm{N\,m/rad}$（终态 $\approx17\,\mathrm{mm}$）。拴定刚度跨越一个数量级，所示记录中的四条曲线接近；C1 的收益在证书（表~\ref{tab:hw-gains}）而非精度。}
