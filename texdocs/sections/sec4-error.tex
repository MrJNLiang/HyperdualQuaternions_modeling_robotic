...
沿用 \cite{P2} 输出 $e_z=[\mathcal O;\mathcal T]\in\mathbb R^6$，$\mathcal O=-\mathrm{Im}\,\tilde r$，$\mathcal T=\tilde p$（$\tilde r,\tilde p$ 为 $\tilde x$ 的旋转/平移分量，$\tilde r=\tilde\eta+\tilde\mu$）。合并范数采用特征长度 $\ell$ 对平移量归一化，即 $\bar{\mathcal T}=\mathcal T/\ell$、$\bar v=v/\ell$；为简洁起见，后文仍记作 $\mathcal T,v$。
...
定理 1 + 定理 2 给出具有级联运动学结构的二阶误差系统：$e_z\xrightarrow{A}e_\xi$，$\dot e_z=Ae_\xi$；$\dot e_\xi$ 的动态由控制律与扰动决定（§5）。误差状态共 12 维，不含加速度层；这是 §4.1 论证的结构性取舍。
