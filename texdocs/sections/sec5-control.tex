...
\textbf{乘性分量与外生分量的分离}。将 $\ddot{\boldsymbol q}_{\mathrm{ref}}=J^{+}(u_{\mathrm{ff}}+u_{\mathrm{fb}}-\dot J\dot{\boldsymbol q})$ 代入 \eqref{eq:5.1}，利用 $JJ^{+}=I_6$ 得扰动分解
\begin{equation}
...
\end{equation}
其中 $\Theta$ 为乘性分量、$d_{\mathrm{ex}}$ 为外生扰动（详见附录 C.1）。$d_{\mathrm{ex}}$ 按时间特性分解为 $d_{\mathrm{ex}}=d_{L_2}+d_b$，前者（噪声型）由定理 3(c) 处理、后者（偏差型）由定理 3(d) 处理。

\textbf{假设集}（定理 3 全文沉默使用，在此一次性列出）：
\begin{itemize}[leftmargin=2em]
  \item \textbf{(A1) 雅可比正则}：在工作集 $\mathcal Q$ 上 $J(\boldsymbol q)$ 行满秩且 $\sigma_{\min}(J)\ge\underline\sigma>0$，从而 $JJ^{+}=I_6$（奇异邻域取阻尼伪逆 \cite{Nak86}，残差归入 $d_{\mathrm{ex}}$）。
...
\end{equation}
$K_p\succ0$ 对称（取 $K_p=k_pI_6$ 即回到标量形式 $\tfrac{k_p}2\|e_z\|^2$）。受 $\tilde\eta=0$ 处 $A$ 奇异与 $SO(3)$ 双覆盖的拓扑障碍所限，以下收敛声明均为工作域（水平集）内的局部结论。
...
\end{equation}
（含扰时右端加 $\dot{\boldsymbol v}_w+\dot{\boldsymbol v}_c$）。将 \eqref{eq:5.2} 代入 \eqref{eq:3.5} 所得 $\dot{\boldsymbol\xi}$ 再代入上式，前馈项与期望/输运项精确相消即得 \eqref{eq:5.5}。
\end{proof}
...
\begin{remark}
水平集条件 \eqref{eq:5.5b} 已由实机核验：G4 档 $c^{*}=72$，实测 $V$ 峰值约 $0.5$，余量逾两个数量级（§6.1）。$\tilde\eta<0$ 分支可将 HDQ 误差整体翻号，不影响 $\tilde{\boldsymbol\xi}$。$\blacksquare$
\end{remark}
...
\textbf{证书参数}：$\kappa>0$ 为误差罚权的倒数（供给率中 $\|e_\xi\|^2$ 项系数 $\tfrac1{2\kappa}$），$\gamma_a>0$ 为待认证的扰动衰减水平；被认证量是 $d\to e_\xi$ 的 $L_2$ 增益 $\gamma^{*}\triangleq\gamma_a\sqrt\kappa$（量纲 s，零初值下 $\|e_\xi\|_{L_2}\le\gamma^{*}\|d_{L_2}\|_{L_2}$）。二者均为分析参数，不进入控制律 \eqref{eq:5.2}；由定理 3(c′) 可反演为整定规则：认证目标 $\gamma^{*}$ 等价于 $K_d\succeq(1/\gamma^{*})I_6$。分量拆分时 $(\kappa_\omega,\gamma_\omega)$、$(\kappa_v,\gamma_v)$ 可独立指定。
...
\begin{proof}
含扰时交叉项仍精确相消，得 $\dot V=-e_\xi^\top K_de_\xi+e_\xi^\top d$；将性能目标写为 $[e_\xi;d]^\top M[e_\xi;d]\ge0$，取 Schur 补即得充要判据 \eqref{eq:5.6a}；分量解耦依赖两处代数恒零（$\mathcal T\times\mathcal T=0$、$\mathcal T\cdot(\mathcal T\times\tilde\omega)=0$）。详见附录 C.7。
\end{proof}
...
\end{equation}
最优点为 $\kappa=\gamma_a^2=1/\lambda_{\min}(K_d)$（\eqref{eq:5.6a} 取等）。等价的\textbf{反演设计规则}：认证任意目标增益 $\gamma^{*}$ \textbf{当且仅当} $K_d\succeq(1/\gamma^{*})I_6$，此时最省证书点为 $(\kappa,\gamma_a)=(\gamma^{*},\sqrt{\gamma^{*}})$。分量拆分判据 \eqref{eq:5.6b} 同理给出 $\gamma^{*}_\omega=1/\lambda_{\min}(K_\omega)$、$\gamma^{*}_v=1/\lambda_{\min}(K_v)$。
\end{theorem}
...
\begin{remark}
证书参数 $(\kappa,\gamma_a)$ 由此压缩为一条换算：认证增益的最优值恰为 $1/\lambda_{\min}(K_d)$。整定入口即阻尼：给定目标衰减，反演得 $K_d$ 下界；极点配置下 $\gamma^{*}=1/(a+b)$。这与一阶 $H_\infty$ 律把 $\gamma$ 写进律内等价，区别只是本文的旋钮为物理增益 $K_d$。该认证值不含保守性：恰为闭环线性化的真实 $H_\infty$ 增益（推导见附录 C.9）。$\blacksquare$
\end{remark}
...
\tag{5.7}\label{eq:5.7}
\end{equation}
即偏差型（$L_\infty$）不确定性不破坏有界性，仅按 $D/\lambda_{\mathrm{eff}}$ 抬高 twist 误差的均方稳态水平；乘性分量的作用（以 $\alpha\lambda_{\max}(K_d)$ 折减有效阻尼、以 $\alpha\lambda_{\max}(K_p)\|e_z\|$ 抬高等效扰动幅值）在 $\alpha\to0$ 时消失。位姿误差 $e_z$ 的稳态量级由静态刚度标度律（稳态残差正比于扰动、反比于刚度）给出准静态估计。
\end{theorem}
...
\begin{remark}
负项只含 $-\|e_\xi\|^2$（扰动到 $e_z$ 的相对阶为 2），故为 RMS 界而非逐点 ISS 界；严格化见附录 C.4，$\Omega_c$ 前提的核验见 §6.1。$\blacksquare$
\end{remark}
