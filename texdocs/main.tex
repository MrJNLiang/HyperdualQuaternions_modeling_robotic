...
\begin{abstract}
\noindent
针对对偶四元数误差体系仅覆盖位姿与速度两层、难以接入加速度级动力学控制的问题，提出一种基于三阶对偶四元数（TODQ）\cite{Con25} 的机械臂几何一致误差体系与 H∞ 计算力矩控制方法：正运动学一次连乘同时输出位姿、速度与任务空间加速度；在两项 HDQ 元素上建立的误差体系经一次乘法同时给出右不变位姿误差与几何一致 twist 误差，且闭环误差动力学为严格级联标准形。据此构造几何一致计算力矩律，闭环满足精确耗散等式，从而同时获得无扰收敛、$L_2$ 扰动衰减的充要判据与偏差型扰动的均方界。其中 $L_2$ 增益可显式最优化：认证值恰为最小阻尼的倒数，且与线性化闭环的真实 H∞ 范数相等，扰动衰减指标因此可直接换算为阻尼增益下界。六自由度机械臂的 33 次实机运行显示，四种控制律在相同增益预算下跟踪精度重合，而仅本文控制律的衰减证书全程成立，认证增益可由 $1/6\,\mathrm{s}$ 定量收紧至 $1/15\,\mathrm{s}$。
\end{abstract}
