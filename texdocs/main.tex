...
\begin{abstract}
\noindent
对偶四元数（DQ）为机械臂位姿提供全局无奇异参数化，但现有框架仅携带零阶信息，向动力学接口延伸时存在位姿/速度误差分离、加速度扰动无入口、偏差型不确定性不满足 $L_2$ 假设三个结构性缺口。本文引入\textbf{三阶对偶四元数}（TODQ）代数 $\mathcal A_2$ \cite{Con25}，使正运动学一次连乘同时输出位姿、twist 与任务空间加速度；误差体系定义在两项 HDQ 元素上，一次乘法同时生成右不变位姿误差与几何一致 twist 误差（定理 1），并闭合为严格级联误差运动学（定理 2）。几何一致计算力矩律使闭环满足精确耗散等式，据此给出无扰时的水平集不变性与渐近收敛、$L_2$ 扰动下 H∞ 增益的 Schur 补判据、$L_\infty$ 扰动下 twist 误差的均方极限界（定理 3）。7 自由度机械臂力矩模式仿真验证了静态刚度标度律与上述性能保证。
\end{abstract}
...
\input{sections/sec6-simulation}
