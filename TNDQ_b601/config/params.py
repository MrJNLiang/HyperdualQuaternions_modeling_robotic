...
CUBE_SIZE = 0.045                          # 立方体边长 [m]（4.5 cm）
...
# --- 任务路标（世界系 gripper 原点；v12b 回归 v9 物理验证几何） ---
GRASP_TCP = CUBE_POS.copy()                # 指垫中心 ≈ 方块中心（设计点）
# v12c 深度（保持/探针两轮裁决后的几何）：
#   p = CUBE + 0.028 x_g - 0.042 z_g（沿 -z_g 移 gripper = 方块在
#   gripper 系抬高：方块中心 c_z=+42 mm）。事实链：
#   (1) diag_hold_grasp：开度 130 保持零蠕变（细扫渐移是 drive
#   切换瞬态）；70→114 之间必有结构碰方块并推飞；
#   (2) diag_innerface z=0 行：垫面内面 ≈ stroke±几 mm（平行夹持
#   可行，接触开度 ≈ 45~65 mm）；z=-20 行另有静态结构占 y∈[0,24]；
#   (3) STL：刀片/罩结构 z 带 ±19.6 mm 全深贯穿、内伸超垫面。
#   方块竖直棱 z_g 投影 = c_z ± 19.5 mm，取 c_z=+42 得
#   [+22.5, +61.5]：竖直棱恰出刀片带上缘 2.9 mm，垫面（z 带内）
#   夹方块侧面下半段；原点在方块中心沿 z_g 下方 4.2 cm（约方块
#   底面高度，插入段沿 x_g 斜下进入不碰柱）。z 向裕量偏紧
#   （2.9 mm），若 exp1 首跑刀片擦角点，再减 GRIPPER_GRASP 或
#   加大负偏置重扫。
GRASP_POS = (CUBE_POS + 0.028 * TOOL_AXIS - 0.042 * _TOOL_Z)
                                            # 抓取位 gripper 原点（v12c 裁决几何）
...
# 上限即 0，纯零位恰在限位边界（首步即触发超限终止），故向内微偏
# 0.04 rad（2.3°，视觉与零位不可辨）。内偏须严格大于
# check_joint_limits 的 margin=0.02（曾取 0.02 恰在判据边界，Isaac
# 读回数值抖动即触发 t=0 abort）。旧 Q_INIT（hover 上方 IK 解，
# q2=-2.6 深折叠）起点即贴近锁滞区，已废弃。
Q_INIT = np.array([0.0, -0.04, -0.04, 0.0, 0.0, 0.0])
...
ISAAC_RENDER_DT = 1.0 / 60.0               # 渲染步长 [s]
...
GRIPPER_OPENING = 0.14                     # 接近指间开度目标 [m]（set_gripper
                                            ...
                                            #   每侧净空 ~48 mm；三跑实测
                                            #   插入段方块零扰动、无自碰）
GRIPPER_GRASP = 0.063                      # 夹持指间开度目标 [m]（v12c 实测：
                                            #   方块面深度 x≈-28 mm 处垫/轨内面
                                            #   ≈ frame-10.7 mm，接触 stroke≈33.2
                                            #   （开度 66 mm）——diag 实测闭合在
                                            #   stroke 33.8/34.4 停驻即夹住。
                                            #   目标 63 mm = 接触点过盈 ~1.5 mm/侧，
                                            #   KP*过盈 ≈ 3 N/指，摩擦 2μN ≈ 3.6 N
                                            #   > 方块自重 0.98 N。★目标不得越过
                                            #   接触点（旧 0.040/0.045 使 drive
                                            #   强闭 10+ mm 把方块挤飞））
...
CTRL_USE_REAL_RATE = False                 # True = 按 CTRL_DT 降采样（真机
                                            #   口径）；False = 每物理步控制
                                            #   （仿真性能上界）
