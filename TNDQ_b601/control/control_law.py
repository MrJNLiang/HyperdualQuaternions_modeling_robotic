...
def dynamic_consistent_inverse(J, M, damping=1e-6):
    """
    Dynamically consistent generalized inverse (Khatib):

        J^bar = M^-1 J^T (J M^-1 J^T + lambda^2 I)^-1

    力矩级移植必备（Isaac B601 实验层）：无加权 J^+ 会把任务误差
    投影到轻量腕关节方向（B601 M55/M66 ≈ 3e-4 kg m^2），qddot_ref
    巨大但 tau = M qddot_ref ≈ 0（diag_wrist_stall：‖q̈_ref‖=24、
    ‖M̂q̈_ref‖=0.28、腕误差永不修正）；J^bar 满足 J J^bar = I 且
    按 M 加权最小化 ‖tau‖，力矩级被控对象实际获得的关节加速度与
    任务意图一致。纯仿真（加速度级被控对象）不受影响——run_lib 在
    力矩级后端时才传入 M。
    """
    J = np.asarray(J, dtype=float)
    ...
    JMinvT = J @ Minv @ J.T
    return Minv @ J.T @ np.linalg.inv(JMinvT + (damping ** 2) * np.eye(m))
...
def geometric_computed_torque_law(err, xi_d_vec6, xi_dot_d_vec6,
    ...
    M              : optional mass matrix. When given (torque-level plant),
                   the joint-space mapping switches from damped_pinv to the
                   dynamically consistent inverse (see dynamic_consistent_inverse).
    ...
    if M is None:
        ...
        qddot_ref = dynamic_consistent_inverse(J, M, damping=damping) @ u_task
