...
def main():
    ...
    # HDQ computation mode:
    #   "chain"    = pure HDQ-POE chain propagation, no explicit full Jacobian is formed.
    #   "jacobian" = old version: compute xi = J(q) qdot first, then build x_dot.
    # Use "chain" for the DQ-vs-HDQ comparison you want now.
    hdq_method = "chain"
    ...
    try:
        ...
        while True:
            ...
            x_model = dq_out["x"]
            J_model = dq_out["J"]
            xi_dq = dq_out["xi"]      # DQ route: FK(q), J(q), xi=J(q)qdot
            xi_hdq = hdq_out["xi"]    # HDQ route: HDQ chain, xi=2*x_dot*x*
    ...
    np.savez(
        "results/data/line_tracking_realtime_hdq_chain_result.npz",
        time=t_log,
        ...
        hdq_method=np.array(hdq_method),
    )
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("results/plots/translation_errors_hdq_chain.png", dpi=200)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("results/plots/orientation_errors_hdq_chain.png", dpi=200)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("results/plots/dq_hdq_twist_difference_log_hdq_chain.png", dpi=200)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("results/plots/runtime_dq_hdq_hdq_chain.png", dpi=200)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("results/plots/qdot_command_hdq_chain.png", dpi=200)

    plt.show()

    print("\nFinished.")
    print("HDQ method          =", hdq_method)
