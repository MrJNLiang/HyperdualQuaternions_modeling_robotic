...
# ---------------------------------------------------------------------------
# 基线控制律 C2（忠实 [Ch20] resolved-acceleration 律；S3 对比 --law dq-chandra）
#
# 结构（control/control_law.py::dq_chandra2020_law）：
#   a_cmd = Ad_{x_tilde} ξ̇_d + ad 输运修正 + K_v·ω_e + K_P·vec6(2 ln x̃)
#   u_task = a_cmd − J̇q̇，  q̈_ref = J⁺ u_task
# 其中 ω_e = Ad ξ_d − ξ = −e_ξ（[Ch20] 式 (32)，经伴随搬运的 twist 误差），
# ξ̇_d 与 J̇q̇ 按原文信息集为解析量（无差分）。与 C1 (5.2) 的唯一结构差异
# 是位姿反馈形式：screw-log 整形 K_P·vec6(2 ln x̃) vs Aᵀ 整形
# −Aᵀ(x̃)K_p e_z（后者才有精确耗散等式与定理 3 证书）。
#
# 配平（同预算准则）：近恒等时 ℓ ≡ vec6(2 ln x̃) → [−2O; T]（旋转通道
# φn ≈ 2·Im(r̃) = −2O 自带因子 2，无 C1 的 1/4、也无消融律的 1/2 折减），
# 线性化逐通道为
#   ℓ̈ + K_v ℓ̇ + K_P ℓ = 0
# 与 C1-tuned 极点 {−4,−20}（特征多项式 (s+4)(s+20)）、DC 刚度 80 对齐：
#   K_v = 24 I6,  K_P = 80 I6
# 三律 d -> (O,T) 名义传递函数逐通道恒等，残余差异纯属结构差异。
CH20_K_V = 24.0 * np.eye(6)     # twist 误差增益（作用于 ω_e = −e_ξ）
CH20_K_P = 80.0 * np.eye(6)     # 位姿增益（作用于 vec6(2 ln x̃)）

# ---------------------------------------------------------------------------
# 消融基线 C2-abl（朴素二阶 twist 差律，不对应任何已发表理论）
# —— 仅用于消融 C1 自身结构（--law dq-ctc），不再作为 [Ch20] 的代表：
# [Ch20] 式 (32) 的 twist 误差经 Ad 搬运，[P2] 前馈亦含 Ad，朴素坐标差
# 是两者的共同稻草人。论文 §6.4 相应标注为 C2-abl。
#
# 结构（control/control_law.py::dq_ctc_law）：
#   u_task = ξ̇_d^num + K_d (ξ_d − ξ) + [p_O·O; −p_T·T] − (J̇q̇)^num
# 消融属性：朴素 twist 差（无 Ad 输运 -> §4.1 伪项）、无 Aᵀ 整形
# （Lyapunov 交叉项不消 -> 无定理 3 证书）、前馈/J̇q̇ 数值差分
# （一拍滞后 + 差分噪声，vs TNDQ σ² 通道免构造解析读出 (3.5)）。
#
# 配平：C2-abl 线性化通道为
#   旋转  Ö + K_ω Ȯ + (p_O/2) O = −d_ω/2   （只有 Ȯ=−ω̃/2 一个 ½ 因子）
#   平移  T̈ + K_v Ṫ +  p_T   T = +d_v
# 与 C1 tuned（旋转刚度 p_O/4 = 80）对齐到 (s+4)(s+20)、DC 刚度 80：
